...
//typedef transform_iterator<hex_from_4_bit<string::const_iterator::value_type>, transform_width<string::const_iterator, 4, 8, string::const_iterator::value_type> > string_from_binary;
...
Digest::isZero () const
{
  ...
  return (m_buffer.size() && m_buffer[0] == 0);
}
...
Digest::finalize ()
{
  ...
  int ok = EVP_DigestFinal_ex (m_context, &m_buffer[0], &tmp);
...
Digest::getHash () const
{
  ...
  {
    result += (m_buffer[i] << 8*(sizeof(std::size_t)-i-1));
  }*/
}
