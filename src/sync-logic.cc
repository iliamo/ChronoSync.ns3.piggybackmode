          ...
          if (diffLeaf->getOperation() == UPDATE)
            {
              ...
              std::string data = diffLeaf->getWireData();
              ...
              {
                boost::recursive_mutex::scoped_lock lock (m_stateMutex);
                tie (inserted, updated, oldSeq) = m_state->update (info, seq, data);
              }
...
void
SyncLogic::addLocalNameAndData (const std::string &prefix, const std::string &wireData, uint32_t session, uint32_t seq)
{
  DiffStatePtr diff;
  {
    //cout << "Add local names" <<endl;
    ...
    SeqNo seqN (session, seq);
    m_state->update(info, seqN, wireData);
